...
import {systemCommentApi} from '../api/system/commentApi.js';
import logger from '../utils/logger.js';

export const useGlobalCommentStore = defineStore('globalComment', () => {
	// 评论列表
	const comments = ref([]);
	...
	const processApiResponse = (response, pageSize) => {
		...
		logger.log('全局评论列表获取成功，总数:', total);
	};
	...
	const fetchComments = async (params) => {
		...
		try {
			...
			const response = await systemCommentApi.list(requestParams);
	...
	const updateComment = async (id, body) => {
		const res = await systemCommentApi.update(id, body);
	...
	const deleteComment = async (id) => {
		const res = await systemCommentApi.delete(id);
	 ...
	 * 审核评论（修改状态）
	 * @param {number} id - 评论 ID
	 * @param {number} status - 审核状态：0=待审核，1=已通过，2=垃圾评论
	 * @returns {Promise<Object|boolean>}
	 */
	const approveComment = async (id, status) => {
		const res = await systemCommentApi.approve(id, {status});
		return handleApiResult(res, '评论审核成功: ' + id, '审核评论失败');
	};
